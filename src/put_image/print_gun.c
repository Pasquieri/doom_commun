...
/*   Updated: 2019/04/26 21:22:46 by cpalmier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_gun(t_env *env, int k)
{
	...
	while (++y < 870)
	{
		...
		{
			i = 4 * x + y * env->m[0].s_l;
			j = 4 * (x - 600 + env->lim_gun[k - 1]) + (y - 665) * env->sp_t[8].s_l; // + lim[0] pour decaler la limite d'avant pour la bonne image
			if (env->sp_t[8].img_str[j + 3] != -1) // si pas transparent
			{
				env->m[0].img_str[i] = env->sp_t[8].img_str[j]; // sp[6] == gun
